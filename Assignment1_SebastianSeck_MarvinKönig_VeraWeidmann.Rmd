...
Likewise we could look at the t-values for significance. In R the significance codes give also a clarification about the coef. significance.
...
Furthermore, we can calculate the Anova of these both models. The result underlines the previous explanation of the non-rejection of $H_0$.
...
In model 2c we can see that the black coefficient is positive now. But this variable interacts with education. This means being black causes an effect of $+0.09-0.023*educ$. So the wage of a black person depends in this case of his education. Model 2a do not integrate the interaction term. Therefore, being black decrease the persons wage by 19%.

$\frac{\delta log(wage)}{\delta black}=\beta_2+\beta_1educ$

However, the standard error for black in model 2c is quite big and we can't verify significance for this coefficient anymore. As already seen in the hypothesis test above ($H0: \beta8 = 0$), education also does not depend on the variable black and the model 2a might be the one which reflects the population in a better way.
...
The value we suggest is equal to the mean of education (13.5). Interpreting models with interactions can be tricky due to the fact that $\beta2$ (coefficient of black) just shows the partial effect of being black when his education = 0. While using the equation black*(educ-c) we centering the variables around the mean value of education. 
