...
#### 1(d)
The unbiased estimator usually implemented is $\frac{SSR}{n-2}$.
...
(sum(mod1c$residuals^2)/(nrow(hprice)-2))
```
...
As mentioned beforehand, through running a linear model, we obtain probabilities less than 0 or greater than 1.
...
####3(v) Fitted values
Wieso machst du bei deinen fitted Values erst die ableitung nach inc? 
...
$40 jaehriger -> Intercept + beta3 * 40 + beta4 * 40^2$ ?? oder?   

$\frac{\delta log(wage)}{\delta black}=\beta_5 = -0.003533$
Our base model is being a woman. So being a male will increase the probability of eligibilty for participating by -0.3533%. The prediction of a man to participate in this program is $-0.5063 -0.003533 = -0.509833 = -50.98\%$  
