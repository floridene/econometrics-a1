...
hprice <- read.dta("/Users/marvin/Documents/Hochschule fÃ¼r Wirtschaft & Recht/Business Intelligence & Process Management/Econometrics/Data/hprice1.dta")
wage2 <- read.dta("/Users/marvin/Documents/Hochschule fÃ¼r Wirtschaft & Recht/Business Intelligence & Process Management/Econometrics/Data/WAGE2.DTA")
ksubs <- read.dta("C:/Users/Vera/Documents/02 BIPM/02 Semester/Econometrics/datasets_tutorial1/stata/401ksubs.dta")
```
...
The value we suggest is equal to the mean of education. Interpreting models with interactions can be tricky due to the fact that $\beta2$ (coefficient of black) just shows the partical effect of being black when his eduaction = 0. While using the equation black*(educ-c) we centring the variables around the mean value of education. 
...
####3(iii & v) Interprtation of coef.
Also already mentioned above, we gain propabilities less than 0 or greater than 1 by running a linear model. 

This are the following effects on the response e401k if one predictor x is increased by one unit while holding the other predictors fix (linear model). Furthermore, we calculate the fitted values (Ex. v)

$\frac{\delta e401k}{\delta inc}=\beta_1+\beta_2inc = 0.01245 -0.00006165 * inc $
The effect of one unit increase in income depends on income itself. Saying we have an income about 30,000 we gain an effect of $-1.83705 = -183%$. Therefore, the probability of being eligiable for participating is $-0.5063 -1.83705 = -2.34335 = -234$% (Das stimmt glaube nicht...)

$\frac{\delta log(wage)}{\delta black}=\beta_3+\beta_4age = 0.02651 -0.0003053 * age $
...
40 jähriger -> Intercept + beta3 * 40 + beta4 * 40^2 ?? oder?   

$\frac{\delta log(wage)}{\delta black}=\beta_5 = -0.003533$
Our base model is being a woman. So being a male will in crease the probability of eligibilty for participating by -0.3533%. The prediction of a man to participate in this program is $-0.5063 -0.003533 = -0.509833 = -50.98$%

* Wie zeigt der mit das % Zeichen in der Gleichung an, Sebastian? :) 

####3(iv) Independence of e401k
```{r 3iv}
summary(mod3ii)

#mod3iv <- lm(e401k ~ inc + age + male, data = ksubs)
#summary(mod3iv)
#oder einfacheres model ohne sq??
```
As shown in the output (mod3ii) we can assume that gender is independent from e401k. We test if $H_0: \beta5 = 0$ in a two-sided test using $\alpha = 0.05$. As the p-value of 0.77 is bigger than 0.05 we cannot reject $H_0$. So, the variable male would be independet of the change for participating in the plan or not. 
...
Income:
$H_0: \beta1 = 0, \beta2 = 0$ (two sided test, $\alpha = 0.05)
We reject $H_0$ as both p_values < 0.05. 

Age:
$H_0: \beta3 = 0, \beta4 = 0$ (two sided test, $\alpha = 0.05)
We reject $H_0$ as both p_values < 0.05. 

-> Man könnte bestimmt auch ein Ftest machen?? Aber keine Ahnung :D
